...
if uploaded_file is not None:
    ...
    # Check if all feature columns are entirely NaN in the raw data
    st.subheader("Step 1.5: Validating Feature Data")
    st.write("Checking for valid feature data before proceeding...")
    # Debug: Show the state of TRAINED_FEATURES columns
    st.write("Feature Columns (Raw Data):")
    st.dataframe(df[TRAINED_FEATURES].head())

    # Check if all values in TRAINED_FEATURES are NaN
    if df[TRAINED_FEATURES].isna().all().all():
        st.error("All feature columns (GR, RHOB, RILD, DT, SP, SPOR) are missing or contain only NaN values. Prediction cannot proceed. Please upload an LAS file with at least one valid feature column containing non-NaN data.")
        st.stop()
    ...
    # Additional check after preprocessing (in case scaler introduces issues)
    if df[TRAINED_FEATURES].isna().all().all():
        st.error("After preprocessing, all feature columns (GR, RHOB, RILD, DT, SP, SPOR) are still NaN. Prediction cannot proceed. This might be due to the data containing only NaN values.")
        st.stop()

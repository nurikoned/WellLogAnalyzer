...
if uploaded_file is not None:
    ...
    if analysis_mode == "View available features only (no prediction)":
        ...
        if plot_features:
            ...
            depth_range = st.slider("Select Depth Range (ft)",
                                   min_value=min_depth,
                                   max_value=max_depth,
                                   value=(min_depth, max_depth),
                                   step=10.0)

            # Filter data based on selected depth range
            depth_filtered_df = df[(df['DEPT'] >= depth_range[0]) & (df['DEPT'] <= depth_range[1])]
            ...
            for i, feature in enumerate(plot_features):
                axes[i].plot(depth_filtered_df[feature], depth_filtered_df['DEPT'], label=feature)
        ...
        try:
            ...
            # Add depth range selection with slider
            min_depth = float(df['DEPT'].min())
            max_depth = float(df['DEPT'].max())
            depth_range = st.slider("Select Depth Range (ft)",
                                   min_value=min_depth,
            ...
            # Option to further focus on smaller depth intervals
            if (depth_range[1] - depth_range[0]) > 100:
                use_zoom = st.checkbox("Enable additional zoom control for detailed view")
                if use_zoom:
                    zoom_window = st.slider("Zoom window size (ft)",
                                          min_value=20.0,
                                          max_value=min(500.0, depth_range[1] - depth_range[0]),
                                          value=100.0,
                                          step=10.0)

                    zoom_position = st.slider("Zoom window position",
                                            min_value=depth_range[0],
                                            max_value=depth_range[1] - zoom_window,
                                            value=depth_range[0],
                                            step=10.0)

                    # Apply additional zoom filtering
                    depth_filtered_df = depth_filtered_df[(depth_filtered_df['DEPT'] >= zoom_position) &
                                                         (depth_filtered_df['DEPT'] <= zoom_position + zoom_window)]
            ...
            if plot_type == "Standard log display with lithology":
                ...
                if plot_features:
                    ...
                    # Ensure axes is always a list or array for consistent indexing
                    if len(plot_features) + 1 == 2:  # If we have only one feature + lithology column
                        axes = np.array([axes[0], axes[1]])  # Convert to array for consistent indexing
                    ...
                    # Create color list
                    colors = [lith_colors.get(label, 'black') for label in depth_filtered_df['LITHOLOGY_LABEL']]

                    # Plot lithology with wider bars
                    bar_width = 0.8  # Make bars wider
                    for i, (depth, lithology) in enumerate(zip(depth_filtered_df['DEPT'], depth_filtered_df['LITHOLOGY_LABEL'])):
                ...
                # Create and display the enhanced lithology plot with the filtered depth range
                fig = create_enhanced_lithology_plot(depth_filtered_df)
                st.pyplot(fig)
                ...
                if show_logs:
                    ...
                    if plot_features:
                        ...
                        # Plot each feature with filtered depth range
                        for i, feature in enumerate(plot_features):
                            axes[i].plot(depth_filtered_df[feature], depth_filtered_df['DEPT'], label=feature)
                            axes[i].set_title(feature)
                            axes[i].invert_yaxis()
                        ...
                        for depth, lithology in zip(depth_filtered_df['DEPT'], depth_filtered_df['LITHOLOGY_LABEL']):
                            if lithology != current_lith:
                                if current_lith is not None:
                                    lith_sections.append((start_depth, depth, current_lith))
                                current_lith = lithology
                        ...
                        # Add the last section
                        if current_lith is not None and start_depth is not None:
                            lith_sections.append((start_depth, depth_filtered_df['DEPT'].iloc[-1], current_lith))

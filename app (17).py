...
TRAINED_FEATURES = ['GR', 'RHOB', 'RILD', 'DT', 'SP', 'SPOR']
...
if uploaded_file is not None:
    ...
    # Check if at least two feature columns have non-NaN data
    st.subheader("Step 1.5: Validating Feature Data")
    st.write("Checking for sufficient feature data before proceeding...")
    # Debug: Show the state of TRAINED_FEATURES columns
    st.write("Feature Columns (Raw Data):")
    st.dataframe(df[TRAINED_FEATURES].head())

    # Debug: Check NaN status for each feature
    st.write("NaN Status for Each Feature (True if all values are NaN):")
    nan_status = {feature: df[feature].isna().all() for feature in TRAINED_FEATURES}
    st.write(nan_status)

    # Count how many feature columns have at least one non-NaN value
    num_valid_features = sum(not df[feature].isna().all() for feature in TRAINED_FEATURES)
    st.write(f"Number of valid features (with at least one non-NaN value): {num_valid_features}")

    if num_valid_features < 2:
        st.error(f"Only {num_valid_features} feature(s) out of {len(TRAINED_FEATURES)} ({TRAINED_FEATURES}) contain non-NaN data. At least 2 feature columns are required for prediction. Please upload an LAS file with sufficient data.")
        st.stop()
    else:
        st.write(f"Found {num_valid_features} valid feature(s) with non-NaN data. Proceeding with preprocessing...")
    ...
    num_valid_features_after_scaling = sum(not df[feature].isna().all() for feature in TRAINED_FEATURES)
    st.write(f"Number of valid features after scaling: {num_valid_features_after_scaling}")
    if num_valid_features_after_scaling < 2:
        st.error(f"After preprocessing, only {num_valid_features_after_scaling} feature(s) contain non-NaN data. At least 2 feature columns are required for prediction.")
        st.stop()

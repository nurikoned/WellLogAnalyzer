...
MISSING_VALUE = -999.25  # Define the placeholder value for missing data
...
if uploaded_file is not None:
    ...
    # Replace missing value placeholder (-999.25) with NaN
    st.write("Replacing missing value placeholder (-999.25) with NaN...")
    ...
    # Debug: Show the state of TRAINED_FEATURES columns after replacing -999.25
    st.write("Feature Columns (After Replacing Missing Values):")
    ...
    if num_valid_features < 2:
        st.error(f"Only {num_valid_features} feature(s) out of {len(TRAINED_FEATURES)} ({TRAINED_FEATURES}) contain non-missing data (not -999.25 or NaN). At least 2 feature columns are required for prediction. Please upload an LAS file with sufficient data.")
        st.stop()
    else:
        st.write(f"Found {num_valid_features} valid feature(s) with non-missing data. Proceeding with preprocessing...")
    ...
    if num_valid_features_after_scaling < 2:
        st.error(f"After preprocessing, only {num_valid_features_after_scaling} feature(s) contain non-missing data. At least 2 feature columns are required for prediction.")
        st.stop()
